...
findUserByLogin pool login = do
          res <- liftIO $ fetch pool (Only login) "SELECT cell_phone, email, first_name, last_name, phone, user_name, user_password, user_role, id, gender, address, city FROM profiles WHERE user_name=?" :: IO [(TL.Text,TL.Text, TL.Text,TL.Text,TL.Text,TL.Text,TL.Text,TL.Text, Maybe Integer, TL.Text, TL.Text, TL.Text)]
